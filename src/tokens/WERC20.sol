...
contract WERC20 is IWERC20, ERC20, Ownable {
    ...
    /*
     * Wrap 1 ERC20 token to 1 wERC20
     */
    function wrap(uint256 assetAmount_) external onlyOwner returns (uint256) {
        require(assetAmount_ > 0, "WERC20: can't wrap zero tokens");
        _mint(msg.sender, assetAmount_);
        return assetAmount_;
    }

    function unwrap(uint256 wrappedAmount_) external onlyOwner returns (uint256) {
